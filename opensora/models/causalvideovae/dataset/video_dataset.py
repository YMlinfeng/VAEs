...
class ValidVideoDataset(data.Dataset):
    ...
    def __init__(
        ...
        if len(self.real_video_files) == 0:
            raise RuntimeError(f"[Dataset] No video files found in {real_video_dir}")
    ...
    def _make_dataset(self, real_video_dir):
        ...
        if osp.exists(cache_file):
            ...
            samples = []
            for ext in self.video_exts:
                samples.extend(glob(osp.join(real_video_dir, "**", f"*.{ext}"), recursive=True))
            if self.is_main_process:
                with open(cache_file, "wb") as f:
                    pickle.dump(samples, f)
        return samples
    ...
    def __getitem__(self, index):
        if index >= len(self):
            raise IndexError(f"[Dataset] Index {index} out of range (total {len(self.real_video_files)})")
        ...
        while attempt < max_attempts:
            try:
                video_path = self.real_video_files[current_index]
                video_tensor = self._load_video(video_path)
                video_tensor = self.transform(video_tensor)
                video_name = os.path.basename(video_path)
                return {'video': video_tensor, 'file_name': video_name}
            except Exception as e:
                print(f"[Dataset] Failed to load video at index {current_index}: {e}")
                attempt += 1
                current_index = 0  # fallback to a known good index

        raise RuntimeError(f"[Dataset] Failed to load video after {max_attempts} attempts (initial index={index})")
    ...
    def _load_video(self, video_path, sample_rate=None):
        ...
        try:
            ...
            raise Exception(f"Decoder failed to load '{video_path}': {e}")

        total_frames = len(decord_vr)
        required_frames = sample_rate * num_frames

        if total_frames < required_frames:
            raise Exception(f"Video too short: {total_frames} frames, requires at least {required_frames}.")
        ...
        try:
            ...
            raise Exception(f"Failed to get frames from '{video_path}': {e}")

        video_tensor = torch.from_numpy(video_data).permute(3, 0, 1, 2)  # (C, T, H, W)
        return video_tensor